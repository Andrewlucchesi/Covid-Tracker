...
export const submitReport = (report) => {
  return (dispatch, getState,{ getFirebase, getFirestore }) => {
    ...
    if(report.soreThroat && report.soreThroat === true){
      batch.set(statsRef, { soreThroatCount: increment }, { merge: true } );
    }
    
    if(report.breath && report.breath === true){
      batch.set(statsRef, { breathCount: increment }, { merge: true } );
    }
    
    if(report.fatigue && report.fatigue === true){
      batch.set(statsRef, { fatigueCount: increment }, { merge: true } );
    }
    
    if(report.taste && report.taste === true){
      batch.set(statsRef, { tasteCount: increment }, { merge: true } );
    }

    if(report.muscle && report.muscle === true){
      batch.set(statsRef, { muscleCount: increment }, { merge: true } );
    }
